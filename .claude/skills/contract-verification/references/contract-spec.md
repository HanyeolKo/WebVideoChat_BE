# WebVideoChat FE↔BE 계약 명세 (BE 거울 / 기준선)

> 이 문서는 BE `contract-qa`가 검증 기준선으로 쓰는 계약 명세다. **상위 관제탑 정본의 거울(mirror)** 이다 — 단일 진실은 `WebVideoChat/.claude/skills/contract-sync/references/contract-spec.canonical.md`이고, 이 파일은 그 사본이다.
> 계약 변경 시 **이 거울만 고치지 마라.** 관제탑 `contract-steward`(또는 `contract-sync` 스킬)를 통해 **정본 먼저 → BE·FE 두 거울 동기화 → 양쪽 contract-qa 재검증** 순서로 진행한다. 관제탑 없이 레포 단독 실행 중이면 거울을 갱신하되 "정본·FE 거울 동기화 필요"를 보고에 명시한다.
...
- BE `SocketHandler.handleTextMessage`는 payload를 파싱하지 않고 그대로 중계하므로 BE는 봉투 변경에 영향받지 않는다.
...
1. ✅ **해소**: BE prod CORS 매핑 추가(`application-prod.yml`, 폴백 기본값 포함) → `.env`의 `CORS_ALLOWED_ORIGINS`가 실제 적용됨.
2. ✅ **해소**: FE→BE 연결을 상대경로 + 호스트 nginx 프록시 모델로 확정. 단, **호스트 nginx에 `/api`·`/socket` → BE 프록시 설정이 반드시 있어야 함**(서버 측).
3. ✅ **해소**: FE `$IMAGE_TAG` 미정의 버그 → SSH 배포 단계 제거(GHCR push 모델)로 소멸.
