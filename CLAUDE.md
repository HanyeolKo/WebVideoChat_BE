...
| 2026-06-07 | 하네스 확장 2건: (1) `pr-merge-flow` 스킬 + `pr-reviewer`(opus) 에이전트 추가 — PR 생성→적대적 검수(동시성/스레드 안전성 축 포함)→blocker 수정(최대 2R)→조건부 자동병합(blocker 0+CI green, main 병합=운영배포라 게이트). (2) cicd-management에 `deploy-symptom-triage.md` 추가 — 배포 증상 디버깅 시 레포 밖 점검(nginx WS 프록시·CORS·프로파일·env·동시성) 체크리스트 자동 생성. be-harness 라우팅·description 갱신 | pr-reviewer.md·pr-merge-flow·cicd-management/references·be-harness | FE와 대칭으로 PR 검수·병합 자동화 + 배포 증상 레포 밖 원인 누락 방지 |
